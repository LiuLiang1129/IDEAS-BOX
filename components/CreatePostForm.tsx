...
import { ImageIcon, PaperPlaneIcon } from './icons';
...
const CreatePostForm: React.FC<CreatePostFormProps> = ({ onSubmit, onViewImage }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-6 rounded-xl shadow-lg space-y-4">
      <textarea
        ...
        rows={4}
        required
        className="w-full p-3 bg-slate-50 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-sky-500 transition"
      />
      {imagePreview && (
        <div className="relative">
          ...
          >
            <img src={imagePreview} alt="Preview" className="w-full h-auto max-h-60 object-contain rounded-lg pointer-events-none" />
          </button>
           <button 
             type="button" 
